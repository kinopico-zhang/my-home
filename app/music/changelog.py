...
VERSIONS: Final[list[ChangelogVersion]] = [
    ChangelogVersion(version="1.7.0", date="2026-09-16", items=[
        ...
        ChangelogItem(kind="修复", text="浏览器的前进键按了没反应的问题修好了:"
                                       " 前进能重新打开刚收起的播放页,"
                                       " 和返回键一来一回, 页面状态完全对得上"
                                       " (应用现在把前进/后退都当成自己的翻页管)"),
        ChangelogItem(kind="修复", text="返回时气泡钉死这件事, 对应用自己的右滑返回和浏览器的返回/前进键"
                                       " 都成立了; 唯一例外是 iPhone 浏览器自带的屏幕边缘返回 ——"
                                       " 那是系统拿整页截图在滑, 苹果不开放给任何网页改动"
                                       " (想连它也没有, 从主屏图标打开应用即可)"),
