...
VERSIONS: Final[list[ChangelogVersion]] = [
    ChangelogVersion(version="2.5.0", date="2026-09-13", items=[
        ...
        ChangelogItem(kind="新增", text="记账的金额输入换成自带计算器键盘 (照着挖财记一笔的样子): 0-9 加加减乘除顺手就能算 (买菜 3.5×2 这种), 边打边出结果; 键盘上的「完成」直接保存, 「再记」存完接着记下一笔, 长按 ⌫ 一键清空; 系统键盘不再弹出来"),
        ChangelogItem(kind="新增", text="记账类别换成完整两级类别: 大类是图标格子, 点一下出子类 (餐饮 → 早餐 / 午餐…, 交通 → 充电 / 地铁…), 收入支出各一套, 类别是从挖财账本导过来的 160 多个"),
