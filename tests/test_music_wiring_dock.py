...
def test_music_dock_wiring():
    ...
    (播放列表/专辑/艺人/已下载/设置), 带缩放上弹动画; 搜索键直进搜索页
    顺手聚焦输入框。船坞本体透明: 只管定位, 点击穿到内容上。"""
    ...
    assert "--dock-clear: calc(var(--dock-h) + 46px + env(safe-area-inset-bottom));" in html
    ...
    assert "rgba(44,44,46,.6);" in keys_css                # 磨砂配方与气泡同款
    ...
    # 上弹菜单: 五项带图标, 从键上方弹出 (缩放+上移入场动画)
    for entry, label in [("playlists", "播放列表"), ("albums", "专辑"),
                         ("artists", "艺人"), ("downloads", "已下载"),
                         ("settings", "设置")]:
    ...
    assert "bottom: calc(var(--dock-h) + 22px + env(safe-area-inset-bottom));" in pop_css
...
def test_music_top_fallback_removed():
    ...
    #top-shield, 首帧 147px 兜底 —— 连脚本带样式一起撤净; 顶部让位回归
    纯 env (谎报 0 的系统里让位失效, 带子照糊, 认了)。"""
    html = music_page_shell()
    js = music_browser_js()
    ...
    # 三处让位 (main / 二级页顶衬 / 播放页抓手) 全是纯 env
    main_css = html[html.index("main {"):html.index("#root-view {")]
    assert "margin-top: env(safe-area-inset-top);" in main_css
    scroll_css = html[html.index(".push-pane .pane-scroll"):html.index(".sticky-head")]
    assert "calc(env(safe-area-inset-top) + 14px)" in scroll_css
    grab_css = html[html.index("#fp-grab {"):html.index("#fp-grab span")]
    assert "calc(env(safe-area-inset-top, 0px) + 3px)" in grab_css
    # 首帧兜底的独立模式媒体查询也撤了
    assert "@media (display-mode: standalone) and (orientation: portrait)" not in html
    # 脚本清单随行: 40 个模块全带版本参数 (1.8.0 起全 v=2)
