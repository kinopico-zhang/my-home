...
def test_bookkeeping_page_has_amount_keyboard():
    ...
    assert 'id="grab-zone"' in html and 'touch-action: none' in html  # 把手下拉关闭
    assert 'filter: grayscale(1)' in html          # 类别图标单色化
    assert 'max-height: 34dvh' in html             # 格子区限高自滚, 弹层一屏放下
